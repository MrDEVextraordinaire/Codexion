...
t_entry heap_pop(t_heap *heap)
{
    ...
    while (1)
    {
        ...
        int index_of_smallest_priority_node = parent;

        // find the index_of_smallest_priority_node among parent and its two children
        if (left  < heap->count && heap->slots[left].priority  < heap->slots[index_of_smallest_priority_node].priority)
            index_of_smallest_priority_node = left;
        if (right < heap->count && heap->slots[right].priority < heap->slots[index_of_smallest_priority_node].priority)
