...
void	push(int priority, char name[100])
{
	...
	task = {priority, ORDERCOUNTER++, name};
...
void	main(void)
{
	t_task	example_tasks[sizeof(t_task) * 100] = {{2, "Send email report"}, {1,
			"Process emergency alert"}, {3, "Backup database"}, {1,
			"Handle server crash"}, {2, "User password reset"}, {3,
			"Clean temp files"}, {1, "Security breach log analysis"}, {2,
			"Generate analytics dashboard"}, {3, "Update documentation"}, {2,
